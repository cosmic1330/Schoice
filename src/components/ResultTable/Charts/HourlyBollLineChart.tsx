...
const HourlyBollLineChart = ({
  ...
  useEffect(() => {
    ...
    const sqlQuery = `SELECT hourly_deal.ts, ${BollIndicatorColor.map(
      ...
      ","
    )} FROM hourly_deal JOIN hourly_skills ON hourly_deal.ts = hourly_skills.ts AND hourly_deal.stock_id = hourly_skills.stock_id WHERE hourly_deal.stock_id = '${stock_id}' AND hourly_deal.ts <=  '${t}1400' ORDER BY hourly_deal.ts DESC LIMIT ${hourly_count}`;
