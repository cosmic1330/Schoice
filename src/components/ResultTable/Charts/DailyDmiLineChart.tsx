...
import { daily_count, DmiIndicatorColor } from "./config";

const DailyDmiLineChart = ({
  stock_id,
  t,
}: {
  stock_id: string;
  ...
  useEffect(() => {
    if (!stock_id) return;
    const sqlQuery = `SELECT daily_skills.t, ${DmiIndicatorColor.map(
      (item) => item.key
    ).join(
      ","
...
export default DailyDmiLineChart;
