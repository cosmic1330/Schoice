...
import { DmiIndicatorColor, weekly_count } from "./config";

const WeeklyDmiLineChart = ({
  stock_id,
  t,
}: {
  stock_id: string;
  ...
  useEffect(() => {
    if (!stock_id) return;
    const sqlQuery = `SELECT weekly_skills.t, ${DmiIndicatorColor.map(
      (item) => item.key
    ).join(
      ","
...
export default WeeklyDmiLineChart;
