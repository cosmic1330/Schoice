...
export const DAILY_MAPPING: Record<string, QueryBuilderMappingItem> = {
...
export interface DailySqlOptions {
  conditions: string[];
  dates: string[];
  daysRange?: number;
  stockIds?: string[];
...
export class StockDailyQueryBuilder extends BaseQueryBuilder {
  ...
  public generateSqlQuery({
    ...
    daysRange = 1,
    stockIds,
  }: DailySqlOptions): string {
    const dayJoins = Array.from({ length: daysRange - 1 }, (_, i) => i + 1)
      .map(
        (number) => `
          JOIN daily_deal "${number}_day_ago" ON "0_day_ago".stock_id = "${number}_day_ago".stock_id AND "0_day_ago".t = "${dates[number]}"
          JOIN daily_skills "${number}_day_ago_sk" ON "0_day_ago".stock_id = "${number}_day_ago_sk".stock_id AND "0_day_ago_sk".t = "${dates[number]}"
        `
      )
      .join("");
