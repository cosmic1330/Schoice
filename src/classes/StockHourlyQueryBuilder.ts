...
export const HOURLY_MAPPING: Record<string, QueryBuilderMappingItem> = {
...
export interface HourlySqlOptions {
  conditions: string[];
  dates: number[];
  hoursRange?: number;
  stockIds?: string[];
...
export class StockHourlyQueryBuilder extends BaseQueryBuilder {
  protected mapping = HOURLY_MAPPING;
  ...
  static getSpecificOptions(): Record<string, readonly string[]> {
    return {
      ...
      indicators: Object.keys(HOURLY_MAPPING),
  ...
  public generateExpression(prompt: StorePrompt): string {
    ...
    if (day2 === "自定義數值") {
      ...
      day2Key = `'${this.converthourToNumber(day2)}${day2Mapping.group}'.${
        day2Mapping.key
      }`;
    }
  ...
  public generateSqlQuery({
    ...
    hoursRange = 1,
    stockIds,
  }: HourlySqlOptions): string {
    const hourJoins = Array.from({ length: hoursRange - 1 }, (_, i) => i + 1)
      .map(
        (number) => `
          JOIN hourly_deal "${number}_hour_ago" ON "0_hour_ago".stock_id = "${number}_hour_ago".stock_id AND "0_hour_ago".ts = "${dates[number]}"
          JOIN hourly_skills "${number}_hour_ago_sk" ON "0_hour_ago".stock_id = "${number}_hour_ago_sk".stock_id AND "0_hour_ago_sk".ts = "${dates[number]}"
        `
      )
      .join("");
