...
2. **靈活條件建構 (特殊規則約束)**:
   ...
   - **週線對齊邏輯**: 回測時，系統會自動將日線日期映射至該週的週日，以確保在週中也能抓到當週最新的週結算資料，同時防止引用下一週的未來數據。
...
4. **超小型資料視覺化 (Ultra-Tiny Charts)**:
   - 提供 80x60 尺寸的高資訊量圖表。
   - 預設顯示 K 線圖（Candlestick Chart），遵循台股漲紅跌綠配色慣例。
   - 疊加 MA5, MA10, MA20, MA60 均線以快速同步價量與趨勢狀態。
