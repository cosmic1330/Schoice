...
## 詳細資訊頁面架構 (Detail Page Architecture)
- **模組化指標渲染 (CHART_CONFIG)**: 
  詳細資訊頁面 (`src/pages/Detail/index.tsx`) 採用與輪播元件分離的配置設計 (`src/pages/Detail/constants/chartConfig.tsx`)。所有技術分析指標組件 (例如 CCI、唐奇通道、Bollean、MACD 等) 均在此註冊，透過 `CHART_CONFIG` 統一輪播載入。
- **資料獲取 (Yahoo Finance / SWR)**:
  前端透過 `generateDealDataDownloadUrl` 和 SWR 機制以 `tauriFetcher` 從 Yahoo Finance API 載入時序歷史數據，轉換成統一的交易數據 `{ t, o, c, h, l, v }` 並透過 `DealsContext` 往下傳遞，所有子指標組件共享此一 context，維持良好的相容性與運算效能。
